    ...
    <section className="mx-auto max-w-6xl px-6 py-10">
      <div>
        ...
        <p className="mt-2 text-slate-600">
          A lightweight calendar placeholder before FullCalendar integration.
        </p>
      </div>
          ...
          {weekDays.map((day, index) => (
            <div key={day} className="p-4">
              ...
              <div className="mt-4 space-y-3">
                {reservations
                  .filter((_, reservationIndex) => reservationIndex === index % 3)
                  .map((reservation) => (
                    <div
                      key={reservation.id}
                      className="rounded-md bg-orange-50 p-3 text-sm text-orange-950"
                    >
                      <p className="font-semibold">{reservation.band}</p>
                      <p className="mt-1">{reservation.time}</p>
                    </div>
                  ))}
              </div>
            </div>
